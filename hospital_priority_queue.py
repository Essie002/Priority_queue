...
class HospitalPQ:
    def __init__(self, root):
        ...
        self.max_queue_size = 10  # Maximum limit for the priority queue
    ...
    def create_widgets(self):
        ...
        tk.Label(operations_frame, text="NAME OF PATIENT:").grid(row=0, column=0, pady=5)
        self.name_entry = tk.Entry(operations_frame)
        self.name_entry.grid(row=0, column=1, pady=5)

        # Entry for patient age
        tk.Label(operations_frame, text="AGE OF PATIENT:").grid(row=1, column=0, pady=5)
        self.age_entry = tk.Entry(operations_frame)
        self.age_entry.grid(row=1, column=1, pady=5)

        # creating the buttons for the operations
        tk.Button(operations_frame, text="ADD THE PATIENT", command=self.add_patient, bg='red').grid(
            row=2, column=0, columnspan=2, pady=10)
        tk.Button(operations_frame, text="MIN", command=self.get_min, bg='red').grid(row=3, column=0,
                                                                                                        columnspan=2,
                                                                                                        pady=5)
        tk.Button(operations_frame, text="REMOVE MIN", command=self.remove_min, bg='red').grid(row=4,
                                                                                                                  column=0,
                                                                                                                  columnspan=2,
                                                                                                                  pady=5)
        tk.Button(operations_frame, text="REMOVE THE PATIENT", command=self.remove_patient, bg='red').grid(row=5, column=0, columnspan=2, pady=5)
        tk.Button(operations_frame, text="UPDATE THE PRIORITY", command=self.update_priority,
                  bg='red').grid(row=6, column=0, columnspan=2, pady=5)
        tk.Button(operations_frame, text="IS EMPTY", command=self.is_empty, bg='red').grid(row=7,
                                                                                                              column=0,
                                                                                                              columnspan=2,
                                                                                                              pady=5)
        tk.Button(operations_frame, text="LENGTH OF QUEUE", command=self.queue_length, bg='red').grid(
    ...
    def draw_hospital(self):
        ...
        self.canvas.create_text(hospital_text_x, hospital_text_y, text="HOSPITAL PRIORITY QUEUE", font=('calibri', 18, 'bold'),
                                fill='black')
        ...
        reception_x2 = 250
        reception_y2 = 250
        self.canvas.create_rectangle(reception_x1, reception_y1, reception_x2, reception_y2, outline='black', width=2,
                                     fill='red')
        self.canvas.create_text((reception_x1 + reception_x2) // 2, (reception_y1 + reception_y2) // 2,
                                text="RECEPTION", font=('calibri', 18, 'bold'))

    def add_patient(self):
        if len(self.priority_queue) >= self.max_queue_size:  # check if PQ has reached its maximum limit
            self.show_error(
                f"Cannot add more patients({self.max_queue_size}).")  # if limit is reached
            return
    ...
    def get_min(self):
        if self.priority_queue:  # checks if the PQ is empty, if not
            ...
            self.show_error("Empty priority queue.")  # if empty

    def remove_min(self):
        if self.priority_queue:  # check if PQ is empty, if not
            removed_patient = self.priority_queue.pop(
                0)  # removes patient with the minimum key; the 1st element in the PQ
                    ...
                    if patient[1] == name and patient[0] == age:
                        ...
                        tk.Button(update_window, text="Update",
                                  command=lambda i=i: self.update_priority_confirm(i, new_age_entry.get(),
                                                                                   update_window),
                                  bg='red').grid(row=1, column=0, columnspan=2, pady=10)
    ...
    def visualize_patients(self):
        ...
        for i, (age, name, patient_id) in enumerate(self.priority_queue):
            x = 550 - (i + 1) * 50
            y = 500
            oval = self.canvas.create_oval(x - 20, y - 55, x + 20, y + 15, fill='black', tags=f"patient_{patient_id}")
            text = self.canvas.create_text(x, y - 65, text=f"{age}", font=('calibri', 10), anchor='center',
                                           tags=f"patient_{patient_id}")  # above
            text = self.canvas.create_text(x, y + 30, text=f"{name}", font=('calibri', 10), anchor='center',
                                           tags=f"patient_{patient_id}")  # below
